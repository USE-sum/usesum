...
def start(config_file,
    ...
    def translate():
        ...
        try:
            ...
            if type(inputs[0]['src']) is np.ndarray:
                for i in range(len(translation)):
                    if type(inputs[i]['src']) is np.ndarray:
                        inputs[i]['src'] = inputs[i]['src'].tolist()
            out = [[{"src": inputs[min(len(inputs)-1,i)]['src'], "tgt": translation[i],
                     "n_best": n_best,
                     "pred_score": scores[i]}
                    for i in range(len(translation))]]
        except ServerModelError as e:
