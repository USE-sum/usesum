...
class RNNDecoderBase(nn.Module):
    ...
    def forward(self, tgt, memory_bank, state, memory_lengths=None,
        ...
        if mem_sizes[2] > self.hidden_size:
            memory_bank = memory_bank[:,:, -self.hidden_size:]

        hid_states = state.hidden[0].size()
        if hid_states[2] > self.hidden_size:
            sth0 = state.hidden[0][:,:, -self.hidden_size:]
            sth1 = state.hidden[1][:,:, -self.hidden_size:]
            state.hidden = (sth0, sth1)
