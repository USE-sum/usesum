...
def build_trainer(opt, device_id, model, fields,
    ...
    """
    Simplify `Trainer` creation based on user `opt`s*vvvv                                       888888888888888xz
...
class Trainer(object):
    ...
    def vec_dif_iter(self, j, batch, src, trunc_size, normalization, shuffle_src_order, src_representation, covered_target):
        ...
        src_lengths = src.size()
        # print("vec dif iter datatype "+str(self.data_type)+"   src_lengths= "+str(src_lengths))
        ...
        # if self.n_gpu > 0:
        #     prev_prediction = torch.zeros( (src_lengths[0],512), dtype=torch.float ).cuda() # torch.zeros_like(tgt, dtype=torch.float)
        # else:
        #     prev_prediction = torch.zeros((src_lengths[0], 512), dtype=torch.float)
        ...
        if len(to_iter)>1:
            for i in to_iter:
                ct += 1
                to_compare = src[:,i,:]
                # covered = torch.acos(torch.nn.functional.cosine_similarity(to_compare, batch.tgt[:,j,:], dim=1 ))
                # print("covered "+str(i)+"  targ "+str(j)+"   =   "+str(covered) )
                ...
                # if i == (src_lengths[1] -1):
                #     self.train_loss.add_mse_loss = True
                ...
                if self.grad_accum_count == 1 and (accum_grad>0 and ct % accum_grad ==0):
                    ...
                    if self.n_gpu > 1:
                        ...
                        onmt.utils.distributed.all_reduce_and_rescale_tensors(
                            grads, float(1))
                    self.optim.step(0)
                # str_pp = str(prev_prediction[:, 0:10])
                prev_prediction = outputs.detach()
                # if torch.isnan(prev_prediction[0,0]):
                #     print(" koniec zbawy, nany "+str_pp+"  output: "+str(prev_prediction[:, 0:10]))
                #     exit()
        # print(" final output = " + str(prev_prediction[:, 0:10]) )
        ...
        for batch in true_batchs:
            ...
            for j in range(0, target_size, trunc_size):
                ...
                if use_vecdif_mode:
                    ...
                    #     shuffle_src_order = False
                    #     tgt = torch.squeeze(tgt, 2)
                    # prev_prediction = torch.zeros_like(tgt, dtype=torch.float)
                    # last_stats = None
                    # to_iter  =[x for x in range(src_lengths[1])]
                    # if shuffle_src_order:
                    #     random.shuffle( to_iter)
                    # for i in to_iter:
                    #     outputs = self.model(i, src, prev_prediction)
                    #
                    #     # 3. Compute loss in shards for memory efficiency.
                    #     # if i == (src_lengths[1] -1):
                    #     #     self.train_loss.add_mse_loss = True
                    #     batch_stats = self.train_loss.sharded_compute_loss(
                    #         batch, outputs, None, j,
                    #         trunc_size, self.shard_size, normalization) #, (i == range(src_lengths[1])-1)
                    #     last_stats = batch_stats
                    #
                    #     # 4. Update the parameters and statistics.
                    #     if self.grad_accum_count == 1:
