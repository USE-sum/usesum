...
class CopyGeneratorCriterion(object):
    ...
    def __init__(self, vocab_size, force_copy, pad, eps=1e-20, focal_gamma=0):
    ...
    def __call__(self, scores, align, target):
        ...
        aligned = align.view(-1, 1)
        added_offset = aligned + self.offset
        out = scores.gather(1,  added_offset).view(-1)
        ...
        if self.focal_gamma!=0:
            loss = torch.pow((1 - tmp), self.focal_gamma)*loss
        return loss
