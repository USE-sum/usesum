...
def build_optim(model, opt, checkpoint):
    ...
    lr_to_use = -1.0
    decay_steps_to_use = -1.0
    start_decay_steps_to_use = -1
    if opt.train_from:
        ...
        if opt.learning_rate != 1.0:
            lr_to_use = opt.learning_rate
        if opt.decay_steps != 10000:
            decay_steps_to_use = opt.decay_steps
        if opt.start_decay_steps!=50000:
            start_decay_steps_to_use = opt.start_decay_steps
    else:
        rnn_size = opt.rnn_size
        if opt.rnn_size == -1:
            rnn_size = opt.dec_rnn_size
        optim = Optimizer(
    ...
    if opt.train_from:
        ...
        optim._set_rate(lr_to_use)
        if decay_steps_to_use!= -1.0:
            optim.decay_steps = decay_steps_to_use
        if start_decay_steps_to_use != -1:
            optim.start_decay_steps = start_decay_steps_to_use
...
class Optimizer(object):
    ...
    def increase_step(self, increase =1):
        self._step += increase

    def step(self, move_step_by = 1):
        """Update the model parameters based on current gradients.

        Optionally, will employ gradient modification or update learning
        rate.
        """
        self._step += move_step_by
