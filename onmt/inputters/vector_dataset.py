...
class VectorDataset(DatasetBase):
    """ Dataset for data_type=='vector'
    ...
    @staticmethod
    def make_vectors_examples_nfeats_tpl(src_data_iter, input_path, truncate, side):
        """
        Args:
            text_iter(iterator): an iterator (or None) that we can loop over
                to read examples.
                It may be an openned file, a string list etc...
            input_path(str): path to file or None
        ...
        examples_nfeats_iter = \
            VectorDataset.make_examples(src_data_iter,input_path, truncate, side)
        # else:
    ...
    @staticmethod
    def make_examples(src_data_iter, input_path, truncate, side):
        """
        Args:
            input_path (iterator): iterator of text sequences
            truncate (int): maximum sequence length (0 for unlimited).
            side (str): "src" or "tgt".
        ...
        for index in range(0, len(data)):
            ...
            if side == "tgt":
                if isinstance(x, collections.Iterable):
                    words, feats, n_feats = tuple(x), [], 0
                else:
                    words, feats, n_feats = tuple( np.linspace(x, stop=1,num=1) ), [], 0
            else:
                if truncate:
                    x = x[:truncate]
                words, feats, n_feats = \
                    VectorDataset.extract_vector_features(x)
    ...
    @staticmethod
    def extract_vector_features(tokens):
        """
        Args:
    ...
    @staticmethod
    def get_fields(n_src_features=0, n_tgt_features=0):
        """
        Args:
        ...
        def make_vector(data, empty= None):
            nfft = data[0].size(0)
        ...
        fields["src"] = torchtext.data.Field(
            ...
            sequential = False, batch_first=True) # postprocessing=make_vector,
