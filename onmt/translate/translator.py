...
def build_translator(opt, report_score=True, logger=None, out_file=None):
    ...
    kwargs = {k: getattr(opt, k)
              for k in ["beam_size", "n_best", "max_length", "min_length",
                        ...
                        "window", "image_channel_size", "decoder_type", "use_port", "decoder_type"]}
...
class Translator(object):
    ...
    def translate_batch(self, batch, data, fast=False, i=0):
        """
        ...
        with torch.no_grad():
            if fast:
                ...
                return self._translate_batch(batch, data,i)
    ...
    def _translate_batch(self, batch, data, batch_i=0):
        ...
        if data_type == 'text':
            ...
            # if len(batch.src.size()) ==3:
            #     _, src_lengths, _ = batch.src.size()
            # else:
            #     _, src_lengths = batch.src.size()
            if len(src.size()) < 3:
        ...
        if self.model.encoder is not None:
            ...
            decoder_outputs, dec_states, attns = \
                self.model.decoder(src, memory_lengths=src_lengths)
            out = self.model.generator.forward(decoder_outputs).data
            return out
        ...
        for i in range(self.max_length):
            ...
            for j, b in enumerate(beam):
                debug=False
                b.advance(out[:, j],
                          beam_attn.data[:, j, :memory_lengths[j]], self.fields, data.src_vocabs[batch_i*batch.batch_size+j], debug)
