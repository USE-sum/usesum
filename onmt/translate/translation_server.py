...
class TranslationServer():
    ...
    def start(self, config_file):
        ...
        for i, conf in enumerate(self.confs["models"]):
            ...
            if "use_host" in opt:
                self.use_host =opt["use_host"]
            if "use_port" in opt:
                self.use_port =opt["use_port"]
            if "use_path" in opt:
                self.use_path =opt["use_path"]
            if "data_type" in opt:
                self.data_type = opt["data_type"]
            if "sentence_separator" in opt:
                self.sentence_separator[model_id] = opt["sentence_separator"]
            if "force_target_split" in opt:
                self.force_target_split[model_id] = opt["force_target_split"]
    ...
    def run(self, inputs):
        ...
        if self.models[model_id].data_type=="vector" and self.use_host!="":
            headers = {'content-type': 'application/json', 'Accept-Charset': 'UTF-8'}
            url = 'http://'+self.use_host+":"+str(self.use_port)+"/"+self.use_path
            texts = inputs[0].get("src", 0)
            if self.sentence_separator[model_id]!="" and (self.sentence_separator[model_id] in texts or len(texts)>=1):
                if len(texts)>=1 and isinstance(texts, list):
                    texts = self.clean_cnndm(texts)
                    data={"instances":texts}
                else:
                    data = {"instances": texts.split(self.sentence_separator[model_id])}
                jsonized = json.dumps(data)
                r = requests.post(url, data=jsonized, headers=headers)
                ret_list = json.loads(r.text)["predictions"]
                inputs[0]["src"] = np.array([ret_list])
            else:
                data = {"instances": [texts]}
                r = requests.post(url, data=json.dumps(data), headers=headers)
                ret_list = json.loads(r.text)
                if "predictions" in ret_list:
                    ret_list = ret_list["predictions"][0]
                else:
                    ret_list=[]
                inputs[0]["src"] = np.array([ret_list])
        # else:
    ...
    def run(self, inputs):
        ...
        if self.data_type == "text":
            ...
            try:
                scores, predictions = self.translator.translate(
                    src_data_iter=src,
                    batch_size=self.opt.batch_size, force_target_split = self.opt.force_target_split)
            except RuntimeError as e:
                raise ServerModelError("Runtime Error: %s" % str(e))
