...
def build_save_vectors(src_corpus, tgt_corpus, fields,
                                           corpus_type, opt):
    """
...
def build_save_in_shards_using_shards_size(src_corpus, tgt_corpus, fields,
    ...
    for x in range(num_shards):
        if x*opt.shard_size >= data_len:
            break
        end = min( (x+1)*opt.shard_size, len(src_data))
        logger.info("Splitting shard %d shard end : %d data len = %d " % (x, end, data_len) )
        np.save(src_corpus + ".{0}.npy".format(x), src_data[x*opt.shard_size:end])
        np.save(tgt_corpus + ".{0}.npy".format(x), tgt_data[x*opt.shard_size:end])

    src_list = sorted(glob.glob(src_corpus + '.*.npy'))
    tgt_list = sorted(glob.glob(tgt_corpus + '.*.npy'))
